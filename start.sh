...
if [ "$?" != 0 ]; then
    ...
    if [ -z "$NoDistantHorizons" ]; then
        ...
        DHVersionInfo=$(curl -s -H "Accept-Encoding: identity" -H "Accept-Language: en" -A "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/90.0.4.212 Safari/537.36" "https://api.modrinth.com/v2/project/dhsupport/version" 2>/dev/null)
